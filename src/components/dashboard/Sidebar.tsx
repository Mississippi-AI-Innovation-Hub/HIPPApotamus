...
export default function Sidebar({ userName, userRole, userEmail }: SidebarProps) {
  ...
  const handleSignOut = () => {
    // TODO: Implement real sign out with Auth.js
        ...
        <ul className="space-y-1">
          {navItems.map((item) => (
            <li key={item.label}>
              <a
                href={item.href}
                className={`group flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium text-slate-600 transition-colors hover:bg-teal-50 hover:text-teal-700 ${
