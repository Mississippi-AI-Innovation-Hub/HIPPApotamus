...
const DOC_TYPE_ICONS: Record<PacketDocument["type"], { color: string; bg: string; icon: string }> = {
  ...
  "state-addendum": { color: "#B45309", bg: "#FEF3C7", icon: "M12 21v-8.25M15.75 21v-8.25M8.25 21v-8.25M3 9l9-6 9 6m-1.5 12V10.332A48.36 48.36 0 0 0 12 9.75c-2.551 0-5.056.2-7.5.582V21M3 21h18M12 6.75h.008v.008H12V6.75Z" },
};
...
export default function AuditPacketsPageClient({
  ...
  const scoreColor = useMemo(() => {
    ...
    if (complianceScore >= 50) return { ring: "#B45309", text: "text-amber-700", bg: "bg-amber-50" };
    return { ring: "#B91C1C", text: "text-red-700", bg: "bg-red-50" };
  }, [complianceScore]);
