...
export default function Sidebar({ userName, userRole, userEmail, onNavigate, activeTab }: SidebarProps) {
  const [collapsed, setCollapsed] = useState(false);
  const navItems = userRole === "admin" ? ADMIN_NAV : VENDOR_NAV;

  const handleNavClick = (item: NavItem) => {
    if (onNavigate) {
      // Extract tab name from href query param
      const url = new URL(item.href, "http://localhost");
      const tab = url.searchParams.get("tab") ?? url.searchParams.get("audit") ? "reminders" : "contracts";
      onNavigate(tab);
    } else {
    ...
    >
      ...
      <nav className="flex-1 overflow-y-auto p-3">
        ...
        <ul className="space-y-1">
          {navItems.map((item) => {
            const tabKey = new URL(item.href, "http://x").searchParams.get("tab") ?? (item.href.includes("audit") ? "reminders" : "contracts");
            const isActive = activeTab === tabKey || (item.label === "Dashboard" && activeTab === "contracts");
            return (
            <li key={item.label}>
              <button
                type="button"
              ...
              >
                <svg
                  className="h-5 w-5 shrink-0 text-slate-400 transition-colors group-hover:text-teal-600"
                  fill="none"
                  viewBox="0 0 24 24"
                  strokeWidth={1.5}
                  stroke="currentColor"
      ...
      <div className="border-t border-slate-200 p-3">
        ...
        <button
          ...
          className={`mt-1 flex w-full items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium text-slate-500 transition-colors hover:bg-red-50 hover:text-red-600 ${
            collapsed ? "justify-center" : ""
          }`}
